...
角色状态通过卡片 `variable_schema.json` 自定义定义，引擎提供通用读写工具：

| 能力 | 说明 |
|------|------|
| **自定义属性** | 每张角色卡通过 Zod Schema 定义自己的数值属性、范围、初始值 |
| **实时读写** | AI 通过 `read_state` / `update_state` 工具读取和修改角色状态 |
| **类型校验** | 所有属性更新经 Zod Schema 校验，超出范围自动钳制 |
| **持久化** | 状态通过 session 快照持久化，卡片模板保持只读 |
| **重置** | `/reset` 命令从卡片模板重建所有数值 |
